...
3、在这样的机制下，如果 JavaScript **线程长**时间地占用了主线程，那么渲染层面的更新就不得不长时间地**等待**，界面长时间不更新，会导致页面响应度变差，用户可能会感觉到**卡顿**。
...
requestIdleCallback回调的执行的前提条件是当前**浏览器处于空闲状态**。

requestIdleCallback的作用是在浏览器一帧的剩余空闲时间内执行优先度相对较低的任务。
