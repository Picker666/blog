...
[三次握手](/base/browser/urlToRender.html#三、tcp-三次握手)
...
服务端收到连接请求报文段后，如果同意连接，则会发送一个应答，该应答中也会包含自身的数据通讯**初始序号**，发送完成后便进入 SYN-RECEIVED 状态。
...
ARQ 协议(**自动重传请求**)也就是超时重传机制。通过确认和超时机制保证了数据的正确送达，ARQ 协议包含**停止等待 ARQ** 和**连续 ARQ** 两种协议。
