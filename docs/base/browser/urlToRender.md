...
**域名**就相当于 IP 地址乔装打扮的伪装者，带着一副面具。它的作用就是便于**记忆**和**沟通**的一组服务器的地址。用户通常使用主机名或域名来访问对方的计算机，而不是直接通过 IP 地址访问。因为与 IP 地址的一组纯数字相比，用字母配合数字的表示形式来指定计算机名更符合人类的记忆习惯。但要让计算机去理解名称，相对而言就变得困难了。因为计算机更擅长处理一长串数字。为了解决上述的问题，DNS 服务应运而生。
...
![DNS 查询过程](/blog/images/base/urlToRender2.png)

### 4、小结

浏览器通过向 DNS 服务器发送域名，DNS 服务器查询到与域名相对应的 IP 地址，然后返回给浏览器，浏览器再将 IP 地址打在协议上，同时请求参数也会在协议搭载，然后一并发送给对应的服务器。

接下来介绍向服务器发送 HTTP 请求阶段，HTTP 请求分为三个部分：

* （1）、TCP 三次握手；
* （2）、http 请求响应信息
* （3）、关闭 TCP 连接。

![DNS 解析过程](/blog/images/base/urlToRender3.png)

## 三、TCP 三次握手

在客户端发送数据之前会发起 TCP 三次握手用以同步客户端和服务端的序列号和确认号，并交换 TCP 窗口大小信息。

![三次握手](/blog/images/base/urlToRender4.png)

[TCP建立连接三次握手](/base/browser/TCP.html#建立连接三次握手)
...
![三次握手](/blog/images/base/urlToRender5.png)
...
包含请求的附加信息，由关键字/值对组成，每行一对，关键字和值用英文冒号“:”分隔。

请求头部通知服务器有关于客户端请求的信息。它包含许多有关的客户端环境和请求正文的有用信息。其中比如：

* Host，表示主机名，虚拟主机；
* Connection,HTTP/1.1 增加的，使用 keep-alive，即持久连接，一个连接可以发多个请求；
...
![三次握手](/blog/images/base/urlToRender6.png)
...
![三次握手](/blog/images/base/urlToRender7.png)
...
![响应报文](/blog/images/base/urlToRender8.png)
...
[浏览器渲染原理](/base/browser/renderPrinciple.html#一、html转换为dom树)

浏览器拿到响应文本 HTML 后，接下来介绍下浏览器渲染机制

![响应报文](/blog/images/base/urlToRender9.png)
...
[断开链接四次握手](/base/browser/TCP.html#断开链接四次挥手)

当数据传送完毕，需要断开 tcp 连接，此时发起 tcp 四次挥手。

![断开连接](/blog/images/base/urlToRender10.png)
